...
class En_DeModel_3(nn.Module):
    ...
    def forward(self, en_p, en_ts, de_p, de_ts, mask):
        ...
        de_hx_list = []
        for i in range(5):
            de_hx_list.append(torch.zeros(en_hx.size()).to(args.device))
        de_loc = torch.zeros(en_hx.size()).to(args.device)
        for loc in range(5):
            for i in range(len(self.decoder)):
                if i % 4 == 0:
                    de_loc, de_hx_list[loc] = self.decoder[i](
                        de_x[:, loc, :].reshape(de_x.shape[0], 1, de_x.shape[2]).float() + de_loc, \
                        hx=de_hx_list[loc])
                else:
                    de_loc = self.decoder[i](de_loc)
            hidden_list.append(de_loc)

        # out_put = self.softmax(self.linear(de_x[:, -1, :]))
        out_put = torch.mean(torch.cat(hidden_list, dim=1), dim=1)
        return out_put, hidden_list
